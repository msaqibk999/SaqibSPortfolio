...
export const CircularProgressBar = ({children, progressEndValue, speed , skill}) => {
    ...
    return (
        ...
        <div className={styles.circular_progress} ref={progressBarRef}>
          <div className={styles.value_container}>{children}</div>
            ...
            <div className={styles.info}> <span className={styles.name}>{skill}</span> <span className={styles.percentage}>{progressValue} %</span></div>
        </div>
    );
  };
